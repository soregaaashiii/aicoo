...
module Aicoo
  class ArticleOpportunityAnalyzer
    CALCULATION_VERSION = "article_opportunity_analyzer_v1".freeze
    ...
    def call
      ...
      Result.new(
        ...
        metadata: {
          ...
          "signals" => signals,
          "reason" => primary_opportunity["reason"],
          "opportunity_type" => primary_opportunity["opportunity_type"],
          "expected_effect" => primary_opportunity["expected_effect"],
          "expected_profit" => value,
          "used_gsc" => used_gsc?,
          "used_ga4" => used_ga4?,
          "used_shopclick" => used_shopclick?,
    ...
    def value_model_metadata(value)
      {
        "name" => "article_opportunity_analyzer",
        "legacy_name" => "suelog_article",
        "calculation_version" => CALCULATION_VERSION,
        "formula" => "primary_opportunity.expected_profit_yen",
        "query" => query,
        "opportunity_type" => primary_opportunity["opportunity_type"],
        "expected_effect" => primary_opportunity["expected_effect"],
        "theme_cluster" => theme_cluster,
    ...
    def calculation_reason
      primary_opportunity["reason"].presence || "GSC・GA4・ShopClick・Learningの関係性から改善Opportunityを分析"
    end

    def expected_profit_yen
      @expected_profit_yen ||= primary_opportunity["expected_profit_yen"].to_i
    end
    ...
    def analysis_metadata
      {
        ...
        "summary" => primary_opportunity["reason"],
        "method" => "GSC/GA4/ShopClick/Learningの関係性からOpportunityを先に判定し、改善効果を利益へ変換"
      }
    end

    def signals
      {
        "search_demand" => search_demand_signal,
        "ctr_gap" => ctr_gap_signal,
        "rank_gap" => rank_gap_signal,
        "engagement" => engagement_signal,
        "cta_gap" => cta_gap_signal,
        "internal_transition_gap" => internal_transition_gap_signal,
        "shop_demand" => shop_demand_signal,
    ...
    def opportunities
      @opportunities ||= begin
        rows = [
          title_ctr_opportunity,
          cta_opportunity,
          internal_link_opportunity,
          same_theme_article_opportunity,
          low_priority_opportunity
        ].compact
        rows.sort_by { |row| [ -row["expected_profit_yen"].to_i, -row["confidence"].to_d ] }
      end
    end

    def primary_opportunity
      @primary_opportunity ||= opportunities.first || low_priority_opportunity
    end

    def title_ctr_opportunity
      return unless used_gsc?
      return unless impressions >= 500 && ctr_gap_signal.positive?

      effect_clicks = [ estimated_incremental_clicks, 1.to_d ].max
      profit = opportunity_profit(clicks: effect_clicks, shop_clicks: effect_clicks * observed_shop_click_rate)
      build_opportunity(
        type: "title_ctr_improvement",
        effect: {
          "estimated_ctr_delta" => ctr_lift.to_f.round(4),
          "estimated_incremental_clicks" => effect_clicks.to_f.round(2)
        },
        profit:,
        confidence: confidence_for(:gsc_ga4_learning),
        reason: "表示回数があり、現在CTRがテーマ内の改善余地を下回るため、タイトル改善で追加クリックを狙える"
      )
    end

    def cta_opportunity
      return unless used_gsc? || used_ga4?
      return unless traffic_signal >= 100 && cta_gap_signal.positive?

      recoverable_shop_clicks = traffic_signal * cta_gap_signal
      profit = opportunity_profit(clicks: 0, shop_clicks: recoverable_shop_clicks)
      build_opportunity(
        type: "cta_improvement",
        effect: {
          "traffic_basis" => traffic_signal.to_f.round(2),
          "estimated_shop_click_rate_delta" => cta_gap_signal.to_f.round(4),
          "estimated_incremental_shop_clicks" => recoverable_shop_clicks.to_f.round(2)
        },
        profit:,
        confidence: confidence_for(:gsc_ga4_shopclick_learning),
        reason: "検索流入またはPVに対してShopClickが弱く、CTA改善で店舗送客を増やせる"
      )
    end

    def internal_link_opportunity
      return unless used_ga4?
      return unless engagement_signal >= 0.6.to_d && internal_transition_gap_signal.positive?

      recoverable_transitions = pageviews * internal_transition_gap_signal
      profit = opportunity_profit(clicks: 0, shop_clicks: recoverable_transitions)
      build_opportunity(
        type: "internal_link_improvement",
        effect: {
          "engagement_signal" => engagement_signal.to_f.round(2),
          "estimated_transition_rate_delta" => internal_transition_gap_signal.to_f.round(4),
          "estimated_incremental_internal_transitions" => recoverable_transitions.to_f.round(2)
        },
        profit:,
        confidence: confidence_for(:ga4_shopclick_learning),
        reason: "関連記事の滞在・ engagement はあるが店舗遷移が弱いため、内部リンクで送客改善余地がある"
      )
    end

    def same_theme_article_opportunity
      return unless used_gsc? && used_ga4? && used_shopclick?
      return unless current_ctr >= 0.015.to_d && engagement_signal >= 0.5.to_d && observed_shop_click_rate >= 0.02.to_d

      expandable_clicks = [ impressions.to_d * 0.003.to_d, clicks.to_d * 0.15.to_d ].max
      profit = opportunity_profit(clicks: expandable_clicks, shop_clicks: expandable_clicks * observed_shop_click_rate)
      build_opportunity(
        type: "same_theme_new_article",
        effect: {
          "estimated_incremental_clicks" => expandable_clicks.to_f.round(2),
          "observed_shop_click_rate" => observed_shop_click_rate.to_f.round(4)
        },
        profit:,
        confidence: confidence_for(:gsc_ga4_shopclick_learning),
        reason: "CTR・PV・ShopClickが揃って強く、同テーマ展開で追加記事Opportunityがある"
      )
    end

    def low_priority_opportunity
      build_opportunity(
        type: "low_priority",
        effect: {
          "estimated_incremental_clicks" => 0,
          "estimated_incremental_shop_clicks" => 0
        },
        profit: 0,
        confidence: confidence_for(:low),
        reason: "検索需要・閲覧・送客の関係から、現時点の改善余地は小さい"
      )
    end

    def build_opportunity(type:, effect:, profit:, confidence:, reason:)
      adjusted_profit = (profit.to_d * learning_adjustment).round
      {
        "opportunity_type" => type,
        "signals" => signals,
        "expected_effect" => effect,
        "expected_profit_yen" => adjusted_profit,
    ...
    def opportunity_profit(clicks:, shop_clicks:)
      (clicks.to_d * value_per_click_yen) + (shop_clicks.to_d * value_per_shop_click_yen)
    end

    def search_demand_signal
      return 0.to_d if impressions.zero?

      clamp_decimal(impressions.to_d / 10_000, min: 0, max: 1)
    end

    def ctr_gap_signal
      return 0.to_d if impressions.zero?

      clamp_decimal(ctr_lift / 0.02.to_d, min: 0, max: 1)
    end

    def rank_gap_signal
      return 0.to_d unless position
    ...
    def engagement_signal
      return 0.to_d if pageviews.zero?

      active_rate = active_users.positive? ? active_users / pageviews : 0.to_d
      engagement_rate = average_engagement_seconds.positive? ? average_engagement_seconds / 90.to_d : 0.to_d
      clamp_decimal((active_rate + engagement_rate) / 2, min: 0, max: 1)
    end

    def cta_gap_signal
      return 0.to_d if traffic_signal.zero?

      target_rate = 0.04.to_d
      clamp_decimal(target_rate - observed_shop_click_rate, min: 0, max: 0.04)
    end

    def internal_transition_gap_signal
      return 0.to_d if pageviews.zero?

      target_rate = 0.025.to_d
      clamp_decimal(target_rate - internal_transition_rate, min: 0, max: 0.025)
    end

    def shop_demand_signal
      return 0.to_d if traffic_signal.zero?

      clamp_decimal(observed_shop_click_rate / 0.05.to_d, min: 0, max: 1)
    end
    ...
    def traffic_signal
      @traffic_signal ||= [ clicks.to_d, pageviews ].max
    end

    def observed_shop_click_rate
      @observed_shop_click_rate ||= traffic_signal.positive? ? recent_shop_clicks / traffic_signal : 0.to_d
    end

    def internal_transition_rate
      @internal_transition_rate ||= pageviews.positive? ? article_transitions / pageviews : 0.to_d
    end
    ...
    def average_engagement_seconds
      @average_engagement_seconds ||= first_numeric(ga4_inputs["engagement_seconds"], ga4_inputs["average_engagement_time_seconds"])
    end

    def article_transitions
      @article_transitions ||= first_numeric(ga4_inputs["article_to_shop_transitions"], ga4_inputs["internal_clicks"])
    end

    def recent_shop_clicks
      @recent_shop_clicks ||= first_numeric(shopclick_inputs["recent_shop_clicks"], shopclick_inputs["clicks"])
    end

    def confidence_for(source)
      base = case source
             when :gsc_ga4_shopclick_learning then 0.72
             when :gsc_ga4_learning then used_ga4? ? 0.62 : 0.5
             when :ga4_shopclick_learning then used_shopclick? ? 0.58 : 0.42
             when :low then 0.25
             else 0.35
             end
      base += 0.08 if used_learning?
      [ base, 0.8 ].min.round(2)
    end

    def used_gsc?
      impressions.positive? || clicks.positive?
    end

    def used_ga4?
      pageviews.positive? || active_users.positive? || average_engagement_seconds.positive? || article_transitions.positive?
    end

    def used_shopclick?
      recent_shop_clicks.positive? || first_numeric(shopclick_inputs["matched_shop_count"], shopclick_inputs["shop_count"]).positive?
    end

    def used_learning?
      learning_action_results.any?
    end
    ...
    def confidence
      primary_opportunity["confidence"].to_d
    end
