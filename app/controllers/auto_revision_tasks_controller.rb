class AutoRevisionTasksController < ApplicationController
  ...
  def codex_queue
    ...
    @auto_revision_tasks = codex_queue_scope
      .includes(
        :action_candidate,
        business: :business_execution_profile,
        target_business: :business_execution_profile
      )
      .limit(100)
      .to_a
    ...
    @codex_target_validations_by_task_id = @auto_revision_tasks.index_with do |task|
      task.codex_prompt_target_validation(prompt: task.codex_prompt(rules: prompt_rules))
    end
