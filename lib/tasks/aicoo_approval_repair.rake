namespace :aicoo do
  ...
  desc "Repair SERP/integrated new-business candidates by creating/linking Business without owner approval"
  task repair_serp_new_business_candidates: :environment do
    result = Aicoo::ApprovedNewBusinessCandidateRepairer.call(source: "repair_serp_new_business_candidates")
    result.errors.each do |error|
      warn "failed action_candidate_id=#{error[:action_candidate_id]} #{error[:error_class]}: #{error[:message]}"
    end
    puts "checked=#{result.checked_count} repaired=#{result.repaired_count} skipped=#{result.skipped_count} failed=#{result.failed_count}"
