...
module Owner
  class DashboardControllerTest < ActionDispatch::IntegrationTest
    test "balanced mode orders by total expected value" do
      ...
      assert_includes response.body, "毎日使う運用入口"
      assert_includes response.body, "自動改修ループを開く"
      assert_includes response.body, "新規事業作成を開く"
      assert_select "a[href='#{owner_auto_revision_loop_path}']"
      assert_select "a[href='#{owner_new_business_pipeline_path}']"
    ...
    test "shows owner alerts approval queue and business rankings" do
      ...
      assert_includes response.body, "AutoRevision準備前"
