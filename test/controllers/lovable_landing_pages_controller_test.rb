...
class LovableLandingPagesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "offers the existing result resume action after a static build failure" do
    campaign = @business.business_campaigns.create!(name: "Build Retry Campaign", campaign_type: "seo", status: "active")
    landing_page = @business.business_prototypes.create!(
      business_campaign: campaign,
      name: "Build Retry LP",
      prototype_type: "github",
      location: "https://github.com/example/build-retry-lp",
      status: "active",
      metadata: {
        "role" => BusinessPrototype::EXTERNAL_LANDING_PAGE_ROLE,
        "lp_name" => "Build Retry LP",
        "lp_public_status" => "testing"
      }
    )
    run = AicooLabGenerationRun.create!(
