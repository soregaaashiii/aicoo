...
class AicooDailyRunsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "shows daily run detail" do
    ...
    assert_includes response.body, "Run ID"
    assert_not_includes response.body, "実行サマリー"
    assert_not_includes response.body, "主要カテゴリごとの件数、成功、warning、失敗を確認します。"
    assert_not_includes response.body, "Auto Revisionスキップ"
